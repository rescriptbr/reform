...
module Make = (Lenses: Lenses) => {
  ...
  module Validation = {
    ...
    let string = (~min=?, ~minError=?, ~max=?, ~maxError=?, field) => {
      Belt.Option.(
        [||]
        <? min->map(min => StringMin(field, min, minError))
        <? max->map(max => StringMax(field, max, maxError))
      );
    };
    ...
    let float = (~min=?, ~minError=?, ~max=?, ~maxError=?, field) => {
      Belt.Option.(
        [||]
        <? min->map(min => FloatMin(field, min, minError))
        <? max->map(max => FloatMax(field, max, maxError))
      );
    };
    ...
    let int = (~min=?, ~minError=?, ~max=?, ~maxError=?, field) => {
      Belt.Option.(
        [||]
        <? min->map(min => IntMin(field, min, minError))
        <? max->map(max => IntMax(field, max, maxError))
      );
    };
  };
