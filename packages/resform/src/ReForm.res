...
module Make = (Config: Config) => {
  ...
  type api = {
    ...
    arrayUpdateByIndex: 'a. (
      ~field: Config.field<array<'a>>,
      ~index: int,
      'a,
    ) => unit,
  ...
  let getInitialFieldsState: Validation.schema<'meta> => array<(
    field,
    fieldState,
  )> = schema => {
  ...
  let use = (
    ...
    let (state, send) = ReactUpdate.useReducer(
      ...
      (action, state) =>
        ...
        | SetFieldsState(fieldsState) =>
          Update({...state, fieldsState: fieldsState})
        | ValidateField(field) =>
          SideEffects(
            self => {
              let fieldState =
                schema |> ReSchema.validateOne(
                  ~field,
        ...
        | FieldArrayRemove(field, index) =>
          Update({
            ...
            values: Config.set(
              state.values,
              field,
              Config.get(state.values, field)->Belt.Array.keepWithIndex((
                _,
                i,
              ) => i != index),
            ),
        ...
        | FieldArrayRemoveBy(field, predicate) =>
          Update({
            ...
            values: Config.set(
              state.values,
              field,
              Config.get(state.values, field)->Belt.Array.keep(entry =>
                !predicate(entry)
              ),
            ),
          })
        | FieldArrayUpdateByIndex(field, value, index) =>
          Update({
            ...
            values: Config.set(
              state.values,
              field,
              Config.get(state.values, field)->Belt.Array.mapWithIndex((
                i,
                currentValue,
    ...
    let getFieldState = field =>
      ...
      ->Belt.Option.mapWithDefault((Pristine: fieldState), ((
        _nameField,
        nameFieldState,
      )) => nameFieldState)
    ...
    let validateFields = (fields: array<field>) => {
      let fieldsValidated = ReSchema.validateFields(
        ~fields,
        ~values=state.values,
        ~i18n,
        schema,
